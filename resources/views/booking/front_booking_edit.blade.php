    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            ...
            <div class="col-sm-12 col-lg-8">
                @if ($data)
                    <form action="{{ url('cust/booking/'.$data->id.'/edit') }}" method="POST" >
                        @csrf
                        @method("put")
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" required autocomplete="off" class="form-control checkin-date" name="checkin" id="checkin" value="{{ $data->checkin_date }}" placeholder="yyyy-mm-dd">
                        </div>
                        <div class="mb-3">
                            <label for="checkout" class="form-label">CheckOut Date</label>
                            <input type="date" autocomplete="off" class="form-control checkout-date" name="checkout" id="checkout" value="{{ $data->checkout_date }}" placeholder="yyyy-mm-dd">
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control room-list" id="room" name="room">
                                @foreach ($arooms->original['data']  as $room)

                                <option {{ $room['room']->id == $data->room_id ? 'selected' : ''}} value="{{ $room['room']->id }}"> {{ $room['room']->title }} [ {{ $room['roomtype']->title }} :: {{ $room['roomtype']->price }}] </option>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" autocomplete="off" class="form-control" required id="total_adult" value="{{ $data->total_adult }}" name="total_adult">
                        </div>
                        <div class="mb-3">
